...
class CustomerRegistrationView(View):
    def get(self, request):
        ...
        if request.user.is_authenticated:
            totalitem=len(Cart.objects.filter(user=request.user))
        return render(request,"app/customereegistration.html",locals())
...
class ProfileView(View):
    ...
    def post(self,request):
        ...
        if form.is_valid():
            ...
            reg = Customer(user=user,name=name,locality=locality,mobile=mobile,city=city,district=district,zipcode=zipcode)
            reg.save()
